...
function activate(_) {
	...
	async function saveDoc() {
		...
		try {
			...
			for (let i = 0; i < documents.length; i++) {
				...
				if (!document.uri.fsPath.endsWith('.git') && !document.uri.fsPath.startsWith('\\')) {
					...
					if (fileControlContent[relativePathNoLevels]) {
						...
						if (documentText === lastFileContent){// we can user hashes here
							...
							const diff = Diff.createTwoFilesPatch("temp", "temp", lastFileContent, documentText, '', '', { context: 20 })
      ...
      if (isFileChanged) {
        await GetDiffName({ time, path: userTempTimedFolder})
      }
		}
		catch (e) {
			console.log(e)
	...
	async function GetDiffName(data) {
		...
		contents.forEach((content, idx) => { 
			const diff = Diff.createTwoFilesPatch(content.file, content.file, content.previousContent, content.currentFileContent, '', '', { context: 20 })
			diffString += diff +"\n"
		})
	...
	function updateFolderName(data) {
		var { time, name, oldPath } = data
		name = name.replaceAll(".", "")
		name = name.replaceAll(" ", "%%")
		const finalName = `${time}--${name}`
		const newPath = path.join(oldPath, "../" + finalName)
		fs.renameSync(oldPath, newPath)
		const controlFilePath = path.join(oldPath, "../", "file-control.json")
		const contents = fs.readFileSync(controlFilePath, "utf8")
		const newContents = contents.replaceAll(time.toString(), finalName)
			
		fs.writeFileSync(controlFilePath, newContents)
	}
    ...
    const currentFiles = fs.readdirSync(userTempTimedFolder);
      ...
      currentFiles.map(async file => {
        ...
        if (fileControlContent[relativePathNoLevels]) {
          ...
          if (previousTimestamp) {
            const fileFullPath = path.join(baseFolder, workspaceFolderId, previousTimestamp, relativePathNoLevels);
            await unzip(fileFullPath);
            previousContent = fs.readFileSync(fileFullPath, 'utf-8');
            fs.unlinkSync(fileFullPath);
          }
        }
  
        return { file: file.split('$$').join('/').replace('.gz', ''), currentFileContent, previousContent };
      })
    );
				...
				if (data.command === 'getListOfVersions') {
					...
					if (folderPath) {
						...
						timestamps.forEach((stampFolder, idx) => {
							if (stampFolder !== "file-control.json") {
								obj[stampFolder] = []
								const innerFolder = fs.readdirSync(path.join(folderPath, stampFolder))
								innerFolder.forEach((inner, idx) => {
									obj[stampFolder].push(inner)
								})
							}
						})
						postMessage({ command: 'listVersions', history: obj })
					}
					...
					var diffString = ""
					contents.forEach((content, idx) => { 
						const diff = Diff.createTwoFilesPatch(content.file, content.file, content.previousContent, content.currentFileContent, '', '', { context: 0 })
						diffString += diff +"\n"
					})

					postMessage({ command: 'showCommitDiff', diff: diffString.trim() })
				}
			})
		}
	})
			...
			<style>
				...
				(function() {
					...
					function showVersions(data){
						...
						for(const folder in data){
							dataArr.push({title: folder, data: data[folder]})
						}
						dataArr.reverse()
						...
						dataArr.forEach((el, idx)=>{
							...
							var title = el.title
							var date = el.title
							if (el.title.split("--").length > 1) {
                const splitted = el.title.split("--");
                ...
                if (!title) {
                  title = date;
							...
							var dTime = new Date(parseInt(date))
							const elDate = monthNames[dTime.getMonth()] + " "+ dTime.getDate()+", "+dTime.getFullYear() +" - "+ dTime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })

							const specClass = idx == 0 ? "active" : ""
							const $item = $('<div class="version-list menu-item" data-id="'+parseInt(date)+'"><div class="top-holder '+specClass+'"><div class="div-block-104"><div class="form-block-5 w-form"><div><div class="editable-title w-input">'+title+'</div></div></div><div class="body-1 tranparent">'+elDate+'</div></div></div></div></div>')
							...
							$item.find(".top-holder").on("click", function(e){
								...
								if(!$target.hasClass("option-btn") && !$target.hasClass("radio-button") && !$target.hasClass("restore-popup")){
									...
									postMessage({command:"showDiff", id: el.title});
                  					resetRadios();
								}
						...
						if(dataArr.length){
							postMessage({command:"showDiff", id: dataArr[0].title})
						}
					}
